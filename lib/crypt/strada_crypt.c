...
static void generate_salt_chars(char *buf, size_t len) {
    ...
    if (get_random_bytes(random, len) < 0) {
        /* Fail closed: do NOT fall back to predictable srand()/rand(). The error
         * is recorded in last_error; gen_salt_internal checks it and refuses to
         * return a salt. Zero buf so no uninitialized bytes leak. */
        memset(buf, 0, len + 1);
        return;
    }
...
static const char* gen_salt_internal(const char *algorithm, int rounds) {
    ...
    /* Fail closed: any error above (RNG failure or unknown algorithm) leaves
     * last_error set — never hand back a usable salt in that case. */
    if (last_error[0] != '\0') {
        salt[0] = '\0';
    }
    return salt;
