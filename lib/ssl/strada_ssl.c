...
/* Connect with explicit control over certificate verification. When verify is
 * non-zero (the default for strada_ssl_connect), the server certificate is
 * checked against the system trust store AND the hostname is bound, so the
 * handshake fails on an untrusted, expired, or wrong-host certificate. Pass
 * verify=0 (strada_ssl_connect_insecure) only when you knowingly want to skip
 * validation. */
SSLConnection* strada_ssl_connect_ex(const char *host, int port, int verify) {
    if (!ssl_initialized) {
        strada_ssl_init();
    }
    ...
    if (verify) {
        /* Verify the peer certificate chain against the system trust store. */
    ...
    if (verify) {
        /* Bind the expected hostname so the handshake fails on a wrong-host
         * certificate — SSL_VERIFY_PEER validates the chain but not the name. */
        SSL_set1_host(conn->ssl, host);
    }

    /* Perform SSL handshake (also runs cert + hostname verification when set) */
...
/* Verify the server certificate and hostname against the system trust store. */
SSLConnection* strada_ssl_connect(const char *host, int port) {
    return strada_ssl_connect_ex(host, port, 1);
}

/* INSECURE: skip all certificate/hostname verification. Use only when you
 * knowingly accept the MITM risk (e.g. a self-signed dev endpoint). */
SSLConnection* strada_ssl_connect_insecure(const char *host, int port) {
    return strada_ssl_connect_ex(host, port, 0);
}

/* Create SSL server socket
 * Takes raw C types for extern "C" */
/* Host-aware TLS listener. host semantics match strada_socket_server_host:
 *   "" / "*" / "::"     -> dual-stack IPv6 wildcard (IPv4 + IPv6)
 *   "0.0.0.0"           -> IPv4 wildcard
 *   "::1" / "127.0.0.1" -> that loopback only
 *   any literal         -> bind the resolved address (its family) */
SSLConnection* strada_ssl_server_host(const char *host, int port, const char *cert_file, const char *key_file) {
    if (!ssl_initialized) {
        strada_ssl_init();
    }
    ...
    /* Create + bind the listening socket, host-aware (IPv4/IPv6/dual-stack). */
    char portstr[16];
    ...
    if (getaddrinfo(dual ? NULL : host, portstr, &hints, &res) != 0 || !res) {
    ...
    int fams[2];
    int nf;
    if (dual) { fams[0] = AF_INET6; fams[1] = AF_INET; nf = 2; }
    else      { fams[0] = AF_UNSPEC; nf = 1; }
    for (int fi = 0; fi < nf && sfd < 0; fi++) {
        for (rp = res; rp; rp = rp->ai_next) {
            if (fams[fi] != AF_UNSPEC && rp->ai_family != fams[fi]) continue;
            ...
            if (bind(s, rp->ai_addr, rp->ai_addrlen) == 0 && listen(s, 10) == 0) {
                sfd = s;
                break;
            }
            close(s);
...
/* Backward-compatible wildcard listener (dual-stack: IPv4 + IPv6). */
SSLConnection* strada_ssl_server(int port, const char *cert_file, const char *key_file) {
    return strada_ssl_server_host("", port, cert_file, key_file);
}
...
/* Host-aware variant: strada_ssl_server_host_sv(host, port, cert, key). */
StradaValue* strada_ssl_server_host_sv(StradaValue *host_sv, StradaValue *port_sv, StradaValue *cert_sv, StradaValue *key_sv) {
    char *host = strada_to_str(host_sv);
    int port = strada_to_int(port_sv);
    char *cert_file = strada_to_str(cert_sv);
    char *key_file = strada_to_str(key_sv);

    SSLConnection *conn = strada_ssl_server_host(host ? host : "", port, cert_file, key_file);
