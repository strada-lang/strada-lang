...
int dbi_execute(DbiStatement *sth, StradaValue *params) {
    ...
    switch (dbh->driver) {
            ...
            if (params && params->type == STRADA_ARRAY) {
                ...
                for (size_t i = 0; i < arr->size && i < (size_t)sth->num_params; i++) {
                    ...
                    if (!val || val->type == STRADA_UNDEF) {
                        ...
                        char *str = strada_to_str(val);   /* malloc'd; SQLITE_TRANSIENT copies it */
        ...
        case DBI_DRIVER_MYSQL: {
            ...
            char **bound_strs = NULL;   /* malloc'd param strings; freed after execute */
            if (sth->num_params > 0 && params && params->type == STRADA_ARRAY) {
                binds = calloc(sth->num_params, sizeof(MYSQL_BIND));
                bound_strs = calloc(sth->num_params, sizeof(char*));
                StradaArray *arr = params->value.av;
                ...
                for (int i = 0; i < sth->num_params && i < (int)arr->size; i++) {
                    ...
                    if (!val || val->type == STRADA_UNDEF) {
                        ...
                        char *str = strada_to_str(val);   /* malloc'd; freed after execute */
                        bound_strs[i] = str;
                ...
                if (mysql_stmt_bind_param(stmt, binds) != 0) {
                    set_error(dbh, mysql_stmt_errno(stmt), mysql_stmt_error(stmt));
                    if (bound_strs) { for (int _b = 0; _b < sth->num_params; _b++) free(bound_strs[_b]); free(bound_strs); }
                    free(binds);
                    return -1;
                }
            }
        ...
        case DBI_DRIVER_POSTGRES: {
            ...
            /* +1 for the terminating NUL: with num_params==0 (no '?' in the
             * SQL, e.g. "SELECT 1"/"BEGIN") new_len equalled strlen(sql) and the
             * `*np = '\0'` below wrote one byte past the allocation. */
            size_t new_len = strlen(converted_sql) + sth->num_params * 10 + 1;
            ...
            *np = '\0';
            ...
            if (sth->num_params > 0 && params && params->type == STRADA_ARRAY) {
                ...
                for (int i = 0; i < sth->num_params && i < (int)arr->size; i++) {
                    StradaValue *val = arr->elements[i];
                    if (val && val->type != STRADA_UNDEF) {
                        /* strada_to_str already returns a malloc'd string; the
                         * extra strdup leaked it. Use it directly (freed by the
                         * cleanup loops below). */
                        param_values[i] = strada_to_str(val);
                    }
                }
            }
    ...
    switch (dbh->driver) {
        ...
        case DBI_DRIVER_SQLITE: {
            ...
            if (rc == SQLITE_ROW) {
                ...
                for (int i = 0; i < cols; i++) {
                    ...
                    /* val is a fresh refcount-1 value; _take consumes that +1
                     * (plain push increfs to 2 and leaks one ref per cell). */
                    strada_array_push_take(arr->value.av, val);
                }
                return arr;
            } else {
                sth->finished = 1;
        ...
        case DBI_DRIVER_POSTGRES: {
            ...
            for (int i = 0; i < cols; i++) {
                ...
                strada_array_push_take(arr->value.av, val);   /* consume the +1 (avoid leak) */
            }
...
StradaValue* dbi_fetchall_arrayref(DbiStatement *sth) {
    ...
    while ((row = dbi_fetchrow_array(sth)) != NULL) {
        strada_array_push_take(all->value.av, row);   /* row is owned; consume the +1 */
    }
...
/* Quote a string VALUE as an SQL literal, using the driver's NATIVE escaper
 * (charset-aware) when a live connection is available, else a portable
 * '' / \\ fallback. Caller frees with free(). Mirrors the implementation in
 * lib/DBI.strada — keep the two in sync. */
char* dbi_quote(DbiHandle *dbh, const char *str) {
    if (!str) return strdup("NULL");
    size_t len = strlen(str);

    if (dbh && dbh->connected) {
        switch (dbh->driver) {
#ifdef HAVE_SQLITE3
            case DBI_DRIVER_SQLITE: {
                char *m = sqlite3_mprintf("%Q", str);  /* quoted literal, NULL -> NULL */
                if (m) {
                    char *out = strdup(m);
                    sqlite3_free(m);
                    if (out) return out;
                ...
                if (dbh->conn) {
                    ...
                    if (out) {
                        ...
                        unsigned long n = mysql_real_escape_string(
                            (MYSQL*)dbh->conn, out + 1, str, (unsigned long)len);
                        out[1 + n] = '\'';
                        out[2 + n] = '\0';
                        return out;
                    }
            ...
            case DBI_DRIVER_POSTGRES:
                if (dbh->conn) {
                    char *lit = PQescapeLiteral((PGconn*)dbh->conn, str, len);
                    if (lit) {
                        char *out = strdup(lit);
                        PQfreemem(lit);
                        if (out) return out;
    ...
    /* Portable fallback (no live connection / driver lib not built). */
    {
        int mysql = (dbh && dbh->driver == DBI_DRIVER_MYSQL);
        char *quoted = malloc(len * 2 + 3);
        char *p = quoted;
        *p++ = '\'';
        while (*str) {
            if (*str == '\'') { *p++ = '\''; *p++ = '\''; }
            else if (*str == '\\' && mysql) { *p++ = '\\'; *p++ = '\\'; }
            else { *p++ = *str; }
            str++;
        }
        *p++ = '\'';
...
/* Quote an SQL IDENTIFIER (table/column name): MySQL backticks, others
 * double-quotes; the quote char is doubled inside. Caller frees with free().
 * (Declared in strada_dbi.h but was previously unimplemented.) */
char* dbi_quote_identifier(DbiHandle *dbh, const char *identifier) {
    if (!identifier) identifier = "";
    char q = (dbh && dbh->driver == DBI_DRIVER_MYSQL) ? '`' : '"';
    size_t len = strlen(identifier);
    char *out = malloc(len * 2 + 3);
    char *p = out;
    *p++ = q;
    while (*identifier) {
        if (*identifier == q) { *p++ = q; *p++ = q; }
        else { *p++ = *identifier; }
        identifier++;
    }
    *p++ = q;
...
StradaValue* dbi_column_names(DbiStatement *sth) {
    ...
    StradaValue *arr = strada_new_array();
    for (int i = 0; i < sth->num_columns; i++) {
        strada_array_push_take(arr->value.av, strada_new_str(sth->column_names[i]));
    }
    return arr;
}
...
StradaValue* strada_dbi_connect(StradaValue *dsn, StradaValue *user, StradaValue *pass, StradaValue *attrs) {
    ...
    /* dbi_connect copies what it needs; free the malloc'd strada_to_str results
     * (these leaked one buffer each per connect() before). */
    free((char*)dsn_str);
    free((char*)user_str);
    free((char*)pass_str);
    if (!dbh) return strada_new_undef();

    /* Bounds-check the fixed 256-entry handle table: without this the 257th
     * connect() in a process wrote g_handles[256..] past the array. */
...
StradaValue* strada_dbi_prepare(StradaValue *dbh_sv, StradaValue *sql) {
    ...
    DbiStatement *sth = dbi_prepare(dbh, sql_str);   /* copies the SQL */
    free((char*)sql_str);   /* malloc'd by strada_to_str — leaked before */
    if (!sth) return strada_new_undef();

    /* Bounds-check the fixed 1024-entry statement table: without this the
     * 1025th prepare() in a process wrote g_stmts[1024..] past the array. */
    if (g_stmt_count >= (int)(sizeof(g_stmts) / sizeof(g_stmts[0]))) {
        dbi_free_statement(sth);
        return strada_new_undef();
    }
