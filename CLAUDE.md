...
- **Int-preserving element compound-assign**: `$h{k} += n` / `$a[i] += n` (and `-=`, `*=`) on a *dynamically-typed* hash/array element no longer box a heap double unconditionally. `gen_hash_compound_assign`/`gen_array_compound_assign` emit a runtime check — `STRADA_IS_TAGGED_INT(old) && STRADA_IS_TAGGED_INT(rhs) ? STRADA_MAKE_TAGGED_INT(...) : strada_new_num(...)` — mirroring the plain binary-`+` path, so the common hash-counter idiom (`$count{$k} += 1` in a loop) does zero heap allocation when both operands are ints. Non-int operands still fall back to the boxed-double path (semantics unchanged); `/=` stays float (int/int isn't int in Perl). The RHS is captured into a temp first so the ternary never re-evaluates it
