...
/* Perl-style (floored) integer modulo — result takes the sign of the right
 * operand (-7 mod 3 == 2). Caller ensures b != 0. Mirrors strada_runtime.h. */
...
typedef void StradaFuture;       /* opaque; futures flow as StradaValue* in generated code */
typedef void StradaThreadPool;   /* opaque */
/* FILE: define it exactly as glibc does (opaque struct), and set glibc's guard
 * so a later `__C__`-block #include <stdio.h> reuses ours instead of conflicting
 * (the old `typedef void FILE` clashed with stdio.h's struct definition). For
 * programs that never include stdio.h, FILE stays an opaque pointer target,
 * which is all the runtime API (FILE *fh, strada_new_filehandle) needs. */
#ifndef __FILE_defined
# define __FILE_defined 1
struct _IO_FILE;
typedef struct _IO_FILE FILE;
#endif
/* Standard streams: generated code initializes v_STDIN/v_STDOUT/v_STDERR from
 * these; the symbols resolve to libc at link. */
extern FILE *stdin;
extern FILE *stdout;
extern FILE *stderr;
...
/* MUST match strada_runtime.h field-for-field: the tcc runtime is built with
 * the main header, so generated code (this header) shares this layout at run
 * time. Keep in exact order. */
typedef struct StradaMetadata {
    ...
    uint8_t is_tied;
    uint8_t is_weak;
    uint8_t blessed_immortal;
    uint8_t has_iv_override;
    uint8_t is_hash_locked;
    int32_t regex_pos;
    void *locked_keys;
    StradaValue *glob_scalar_slot;
} StradaMetadata;
...
struct StradaHash {
    ...
    uint32_t *hash_index;       /* open-addressing table */
...
StradaValue* strada_close(StradaValue *fh);  /* returns tagged int: 1 closed, 0 nothing */
...
/* Per-call-site monomorphic dispatch cache; see strada_runtime.h. Must stay
 * field-for-field identical to the definition there (shared runtime ABI). */
...
/* Concat key hash operations - build hash keys in stack buffers */
...
/* ===================================================================== */
/* Synced from strada_runtime.h: declarations the generator emits that were
   missing here (kept in sync with the main header; TCC has no system
   headers so anything needing system types stays out). */
/* ===================================================================== */
...
/* strada_hv_check_lock / strada_hv_store_take are runtime-internal static
 * inlines (hash-lock checks, libc snprintf/strcmp); the generator never emits
 * them, so they are intentionally omitted from this TCC header. */
/* break_self_cycle is a static-inline early-out wrapper in the main header;
