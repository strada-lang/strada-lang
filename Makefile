...
.PHONY: all clean test test-all test-examples test-selfhost test-suite runtime bootstrap compiler examples run run-bootstrap help info selfhost install uninstall tools libs lib-dbi lib-crypt lib-ssl lib-readline configure-check stage1 quick interpreter test-interp xs2strada update-bootstrap
...
ifeq ($(DEV),1)
all: stradac $(RUNTIME_OBJ) strada-driver tools
else
all: stradac $(RUNTIME_OBJ) strada-driver tools libs
endif
	@echo ""
	@echo "✓ Build complete!"
...
# The `strada` driver is itself a Strada program (tools/strada-driver.strada),
# behind the thin `strada` bash shim. Built directly with the freshly-built
# stradac + $(CC) — NOT via the `strada` shim, which would be circular (the
# shim execs this very binary). The driver has no `use`/`-D`, so a plain
# emit-C-then-link is enough.
strada-driver: stradac $(RUNTIME_OBJ) tools/strada-driver.strada
	@echo "=== Building strada-driver (compiled wrapper behind the shim) ==="
	@STRADA_GC=off STRADA_DIR=$(CURDIR) ./stradac tools/strada-driver.strada strada-driver.tmp.c
	$(CC) $(CFLAGS) -Wno-unused-function -o strada-driver strada-driver.tmp.c $(RUNTIME_OBJ) -I$(RUNTIME_DIR) $(LDFLAGS)
	@rm -f strada-driver.tmp.c
	@echo "OK strada-driver built (the bash strada is now a thin shim)"
...
# Converter tools (built separately via make xs2strada)
CONVERTER_BINS = tools/xs2strada
...
install: stradac $(RUNTIME_OBJ) libs tools
	...
	@# Install the compiled driver and the thin shim. The shim's
	@# STRADA_HOME / STRADA_BIN defaults are patched to the install layout
	@# (HOME = lib dir for runtime/lib/config, BIN = bin dir for binaries).
	install -m 755 strada-driver $(INSTALL_BIN)/strada-driver
	@sed -e 's|STRADA_HOME="$${STRADA_HOME:-$$SCRIPT_DIR}"|STRADA_HOME="$${STRADA_HOME:-$(INSTALL_LIB)}"|' \
	     -e 's|STRADA_BIN="$${STRADA_BIN:-$$SCRIPT_DIR}"|STRADA_BIN="$${STRADA_BIN:-$(INSTALL_BIN)}"|' \
	...
	@# Build and install tools (converters excluded)
...
clean:
	...
	rm -rf build/*
	rm -f stradac
	rm -f strada-driver strada-driver.tmp.c
