...
gcc -o hello hello.c runtime/strada_runtime.c -Iruntime -ldl -lm
./hello
```
...
./stradac --help                   # Show help
```

(`./stradac --help` prints the version banner; there is no separate `--version` flag.)
```
...
| `uint8` (or `byte`), `uint16`, `uint32`, `uint64` | `uint8_t`, etc. |
| `size_t` | `size_t` |
...
core::fwrite($fh, "Line 1\n");
core::fwrite($fh, "Line 2\n");
core::close($fh);
```
...
my str $home = sys::getenv("HOME");     # Also works (legacy alias)
...
At compile time, `core::` is normalized to `sys::` with zero runtime overhead. The `sys::` prefix continues to work for backwards compatibility.
...
- **Tagged integers**: Integer values may be tagged pointers (low bit set) rather than heap-allocated structs. Always use `strada_to_int()` to extract values -- it handles both tagged and heap integers transparently. Never dereference a `StradaValue*` directly to read its integer field.
- Use `strada_to_int()`, `strada_to_str()`, etc. to extract values
- Use `strada_new_int()`, `strada_new_str()`, etc. to create values
- Remember to `strada_decref()` before replacing a variable (safe to call on tagged integers -- it's a no-op)
