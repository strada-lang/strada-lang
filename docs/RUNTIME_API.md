...
> **Looking for Strada-level functions?** This document covers the C-level
> runtime that generated code calls into. For the user-facing built-ins
> (`math::sqrt`, `core::open`, `async::send`, etc.) see
> [BUILTIN_FUNCTIONS.md](BUILTIN_FUNCTIONS.md) for the complete alphabetical
> list, or [QUICK_REFERENCE.md](QUICK_REFERENCE.md) for a topic-organized
> cheat-sheet.
...
typedef enum {
    ...
    STRADA_INT,        // 1  64-bit integer
    STRADA_NUM,        // 2  64-bit float
    STRADA_STR,        // 3  String
    STRADA_ARRAY,      // 4  Array
    STRADA_HASH,       // 5  Hash map
    STRADA_REF,        // 6  Reference
    STRADA_FILEHANDLE, // 7  File handle
    STRADA_REGEX,      // 8  Compiled regex
    STRADA_SOCKET,     // 9  Socket
    STRADA_CSTRUCT,    // 10 C struct wrapper
    STRADA_CPOINTER,   // 11 Generic C pointer
    STRADA_CLOSURE,    // 12 Anonymous function with captured environment
    STRADA_FUTURE,     // 13 Async future
    STRADA_CHANNEL,    // 14 Async channel
    STRADA_ATOMIC      // 15 Atomic value
} StradaType;
```

### Tagged Integer Encoding

Integers are encoded directly in the `StradaValue*` pointer using bit tagging, avoiding heap allocation:

```c
// Bit 0 = 1: tagged integer (pointer encodes the value, not a heap address)
...
- `strada_new_int()` returns `STRADA_MAKE_TAGGED_INT(val)` -- no malloc
...
- References to tagged ints are unboxed to heap values in `strada_ref_create()`

### StradaValue Structure

For non-integer values, heap-allocated structs are used:
...
typedef struct StradaValue {
    ...
    union {
        ...
        struct StradaValue *rv;   // STRADA_REF — a flat pointer to the target
        FILE *fh;                 // STRADA_FILEHANDLE
        void *sock;               // STRADA_SOCKET
        void *ptr;                // STRADA_CSTRUCT / STRADA_CPOINTER
    } value;
    size_t struct_size;           // string-length cache (hot path)
    StradaMetadata *meta;         // NULL for most values; holds blessed_package,
                                  // weak/tied flags, etc.
} StradaValue;
```

The reference type (`'$'`/`'@'`/`'%'`) is a constructor argument to
`strada_new_ref()`, not a field on the value; the blessed package name lives in
`meta->blessed_package`.

**Tagged Integer Optimization**: Integers that fit in 63 bits are not stored in a heap-allocated `StradaValue` struct. Instead, they are encoded directly in the pointer with the low bit set (tagged pointer). This means `StradaValue*` pointers may be either real heap pointers or tagged integers. Always use `strada_to_int()` to extract integer values and `strada_new_int()` to create them -- these functions handle both representations transparently. Never dereference a `StradaValue*` directly to read integer fields without first checking `STRADA_IS_TAGGED_INT(sv)`.
...
// Create integer (returns tagged pointer — no heap allocation)
...
**Note:** `strada_new_int()` returns a tagged pointer (`STRADA_MAKE_TAGGED_INT(value)`), not a heap-allocated struct. This means integers have zero allocation cost and are immortal (refcounting is a no-op).
...
```c
// Convert any value to integer (handles both tagged and heap-allocated ints)
...
Reference counting uses atomic operations for thread safety. Tagged integers (bit 0 = 1) are immortal and bypass refcounting:
...
Reference counting uses **atomic operations**, so reference-counted values can be shared across threads, and the async runtime (`async::`) provides a thread pool for parallel work. Two memory features operate in single-threaded scope and self-disable when threading is active: the automatic **cycle collector** and the request **arena**.

## Memory Model

- **Tagged integers**: Integers in the range -(2^62) to (2^62-1) are encoded directly in `StradaValue*` pointers using `(val << 1) | 1`. They require zero heap allocation and zero reference counting (`strada_incref`/`strada_decref` are no-ops). `strada_to_int()`, `strada_to_num()`, `strada_to_str()`, `strada_to_bool()`, and `strada_new_int()` handle tagged integers transparently. Any runtime or `__C__` block code that accesses `sv->type` must check `STRADA_IS_TAGGED_INT(sv)` first to avoid dereferencing the tagged pointer as a memory address.
- **Reference unboxing**: `strada_ref_create()` unboxes tagged ints to heap-allocated values before creating a reference, since references need a stable pointer target.
- **Hash key optimization**: `strada_hash_set_with_hash()` uses `strdup()` instead of `strada_intern_str()` for keys, avoiding intern table overhead for unique concatenated keys (e.g., `"item_" . $i`). `strada_intern_release()` handles both interned and non-interned keys.
- All other `StradaValue` objects (strings, arrays, hashes, refs, nums) are heap-allocated
- Reference counting manages memory automatically for heap-allocated values
...
- Strada is 3.4x faster than Python and 3.5x faster than Perl on array/hash benchmarks thanks to tagged integers
