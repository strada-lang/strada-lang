...
Perl array/hash references map to Strada *references*, so Strada code can use
`$arr->[i]` / `$h->{k}`.

### Objects and Methods

A blessed Strada object returned from the runtime comes back as a
`Strada::Object` that dispatches method calls back into Strada (instead of being
flattened to a plain hashref). Construct objects with `new_object` and call
methods directly:

```perl
my $c = $lib->new_object('Counter', count => 10, name => 'hits');
# (sugar for $lib->call('Counter::new', count => 10, name => 'hits'))

$c->increment;            # method dispatch -> 11
$c->add(5);               # method with arguments -> 16
print $c->describe, "\n"; # "hits=16"
$c->strada_class;         # "Counter"
...
A Strada `throw` raised inside a called function or method is re-raised as a
catchable Perl exception; the runtime recovers cleanly so later calls still
work. A thrown object arrives in `$@` as a `Strada::Object`.

```perl
my $ok = eval { $c->checked_add(-1); 1 };
print "caught: $@\n" if !$ok;   # "negative amount not allowed"
```

### Argument Count

With libffi present (build dependency `libffi-dev`), calls take any number of
arguments. Without libffi the module falls back to a 4-argument limit (calls
with more `croak`).
...
Perla compiles Perl programs directly in one step, with no intermediate Strada source.
...
Strada includes two additional tools for converting code between Strada and Perl:
...
Comprehensive converter that translates Strada source code into equivalent Perl 5 code. Useful for porting Strada projects back to Perl, generating Perl reference implementations, or sharing code with Perl-only teams.
...
These features compile to native C code and require no runtime Perl interpreter. When converting Perl code to Strada manually, these constructs can be used directly.
