...
```strada
length($s)              # Length
length($s)              # UTF-8 codepoint count (use core::byte_length for raw bytes)
substr($s, 0, 5)        # Substring (codepoint offsets, UTF-8 safe)
index($s, "sub")        # Find (codepoint position)
uc($s)  lc($s)          # Case (handles Latin-1 range)
trim($s)                # Whitespace
chomp($s)               # Strip trailing \n or \r\n in-place, returns modified string
join(",", @arr)         # Join array
chr(65)  ord("A")       # Char conversion (chr 0-255 = single byte, 256+ = UTF-8)
reverse("café")         # éfac — codepoint-aware
...
**Files:** `core::open`, `core::open_str`, `core::str_from_fh`, `core::close`, `core::readline`, `core::slurp`, `core::spew`, `core::seek`, `core::tell`, `core::flush`, `core::autoflush`, `core::file_exists`, `core::file_mtime`, `core::stat`
...
my int $val = 10;
__C__ {
    long long v = strada_to_int(val);  // handles tagged integers transparently
    printf("val = %lld\n", v);
    strada_decref(val);                // safe on tagged ints (no-op)
    val = strada_new_int(v * 2);       // returns tagged pointer (no heap alloc)
}
say($val);  # 20
```
...
**Tagged integers**: Integer values are encoded directly in pointers (low bit set) with zero heap allocation. Always use `strada_to_int()` to extract -- never dereference the pointer directly to read integer fields.
...
my str $g = perl5::call("greet", "World");       # Call sub (single string arg)
