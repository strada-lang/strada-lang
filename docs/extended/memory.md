...
Strada uses **automatic reference counting** for memory management, with a key
optimization: **integers are stored as tagged pointers** with zero heap allocation.
You rarely need to think about memory - it's handled automatically.

## Tagged Integers

Integers that fit in 63 bits are encoded directly inside the pointer itself (the
low bit is set to mark it as a tagged integer). This means integer operations --
arithmetic, comparisons, loop counters, array indexing -- require **no memory
allocation and no reference counting**. This optimization is completely transparent
to Strada code. Combined with other optimizations, Strada is 3.4x faster than
Python and 3.5x faster than Perl on array/hash benchmarks.

## How Reference Counting Works

Non-integer values (strings, arrays, hashes, references, floats) have a reference
count tracking how many variables point to them. When the count reaches zero,
...
1. **Integer values** - stored as tagged pointers, zero allocation, no refcounting needed
2. **Variable assignment** - increments refcount (for non-integer values)
3. **Scope exit** - decrements refcount
4. **Reassignment** - decrements old value, increments new
5. **Zero refcount** - memory freed immediately
...
# Ensure capacity for at least N elements (a bare builtin, not under core::)
reserve(@data, 100);
```
...
Plain reference counting can't reclaim reference *cycles* on its own. Strada
ships an automatic **cycle collector** (a Bacon–Rajan trial-deletion collector,
on by default) that reclaims indirect cycles through arrays, hashes, and
references — so the common case below is handled for you, no `core::weaken()`
required.

```strada
# A reference cycle. Plain refcounting alone can't free this, but the default
# cycle collector reclaims it automatically.
my hash %a = {};
my hash %b = {};
$a{"other"} = \%b;
$b{"other"} = \%a;   # Cycle: neither reaches refcount 0 by refcounting alone
```

### Avoiding Circular References

The cycle collector handles most cycles automatically. For cycles it does not reach (closure-capture cycles), for deterministic teardown, or when the collector is configured out, use weak references. A weak reference does not keep its target alive — when the target's last strong reference is dropped, the target is freed and all weak references to it become `undef`.
...
- Use `int` types for counters, indexes, and arithmetic (tagged integers have zero allocation cost)
...
- Use `num` where `int` would suffice (nums are heap-allocated, ints are tagged pointers)
...
|----------|--------------|------|------|
| Strada | Tagged ints + refcounting + cycle collector | Zero-alloc ints, predictable cleanup, reclaims cycles | DESTROY skipped for collected cycles |
| Perl | Reference counting | Mature, well-tuned | All values heap-allocated |
| Python | Refcount + GC | Handles cycles | GC pauses, all values heap-allocated |
...
- Integers use tagged pointers with zero heap allocation and no refcounting overhead
- All other values are managed automatically via reference counting
