...
void my_double_array(StradaValue *arr) {
    ...
    for (size_t i = 0; i < a->size; i++) {
        int64_t val = strada_to_int(a->elements[a->head + i]);
        a->elements[a->head + i] = strada_new_int(val * 2);
    }
}
```
...
char* strada_to_str(StradaValue *sv);  // caller must free()
...
|----------|-------------|
| `strada_to_int(sv)` | Extract int64_t from StradaValue* (handles both tagged and heap ints) |
| `strada_to_num(sv)` | Extract double from StradaValue* |
| `strada_to_str(sv)` | Extract string (must free!) |
| `strada_new_int(i)` | Create StradaValue* from int64_t (returns tagged pointer, no heap alloc) |
| `strada_new_num(n)` | Create StradaValue* from double |
| `strada_new_str(s)` | Create StradaValue* from string |
| `strada_new_undef()` | Return a fresh undef value |

**Tagged Integers**: Integer values are encoded directly in `StradaValue*` pointers with the low bit set. This means `StradaValue*` parameters in `__C__` blocks may be tagged pointers rather than valid heap addresses. Always use `strada_to_int()` to extract integer values -- never dereference the pointer directly. `strada_decref()` and `strada_incref()` are safe to call on tagged integers (they are no-ops).
