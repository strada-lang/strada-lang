...
# Check for errors
if ($lib == 0) {
    die("Failed to load library: " . sys::dl_error());
}
...
if ($func == 0) {
    die("Function not found: " . sys::dl_error());
}
...
```strada
# Absolute path
my int $lib = sys::dl_open("/usr/lib/libfoo.so");

# Relative path
my int $lib = sys::dl_open("./libfoo.so");

# System library (uses LD_LIBRARY_PATH)
my int $lib = sys::dl_open("libfoo.so.1");

# Common system libraries
my int $libm = sys::dl_open("libm.so.6");       # Math
my int $libc = sys::dl_open("libc.so.6");       # C standard
my int $libssl = sys::dl_open("libssl.so.3");   # OpenSSL
```
...
my int $result = sys::dl_call_int($func_ptr, [$arg1, $arg2]);

# double func(double a, double b, ...)
my num $result = sys::dl_call_num($func_ptr, [$arg1, $arg2]);

# void func(args...)
sys::dl_call_void($func_ptr, [$arg1, $arg2]);
```
...
my int $result = sys::dl_call_int_sv($func_ptr, [$sv1, $sv2]);
my str $result = sys::dl_call_str_sv($func_ptr, [$sv1, $sv2]);
sys::dl_call_void_sv($func_ptr, [$sv1, $sv2]);
```
...
func main() int {
    ...
    # Call my_add
    my int $add = sys::dl_sym($lib, "my_add");
    my int $sum = sys::dl_call_int_sv($add, [10, 20]);
    say("10 + 20 = " . $sum);

    # Call my_greet
    my int $greet = sys::dl_sym($lib, "my_greet");
    my str $msg = sys::dl_call_str_sv($greet, ["World"]);
    say($msg);
...
my int $lib = sys::dl_open("libfoo.so");
if ($lib == 0) {
    die("Failed to load: " . sys::dl_error());
}

my int $func = sys::dl_sym($lib, "function");
if ($func == 0) {
    sys::dl_close($lib);
    die("Symbol not found: " . sys::dl_error());
}
```
...
func init() int {
    $lib = sys::dl_open("./libmylib.so");
    if ($lib == 0) {
        return 0;
    }

    $fn_add = sys::dl_sym($lib, "my_add");
    $fn_greet = sys::dl_sym($lib, "my_greet");
...
func add(int $a, int $b) int {
    return sys::dl_call_int_sv($fn_add, [$a, $b]);
}

func greet(str $name) str {
    return sys::dl_call_str_sv($fn_greet, [$name]);
}
```
...
func _load() int {
    ...
    $lib = sys::dl_open("lib/ssl/libssl_wrapper.so");
    if ($lib == 0) {
        return 0;
    }

    $fn_init = sys::dl_sym($lib, "ssl_init");
    $fn_connect = sys::dl_sym($lib, "ssl_connect");
    $fn_read = sys::dl_sym($lib, "ssl_read");
    $fn_close = sys::dl_sym($lib, "ssl_close");
...
func init() int {
    if (!_load()) { return 0; }
    return sys::dl_call_int($fn_init, []);
}

func connect(str $host, int $port) int {
    if (!_load()) { return 0; }
    return sys::dl_call_int_sv($fn_connect, [$host, $port]);
}

func read(int $handle, int $len) str {
    return sys::dl_call_str_sv($fn_read, [$handle, $len]);
}

func close(int $handle) void {
    sys::dl_call_void_sv($fn_close, [$handle]);
}
```
