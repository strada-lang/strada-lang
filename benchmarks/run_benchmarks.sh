...
usage() {
    ...
    echo "  -a, --all       Run all languages (default: strada, go, node, php —"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -l|--list)
            ...
            ;;
        -a|--all)
            LANGS="strada vm perl python ruby go node php"
            shift
            ;;
        --langs)
...
# Compile Go benchmarks ahead of time (timed like Strada: the binary runs,
# not `go run`, so compile cost is excluded). Each .go is a standalone
# stdlib-only package main built to ${bench}_go.
if lang_enabled go && command -v $GO &>/dev/null; then
    echo -e "${CYAN}Compiling Go benchmarks...${RESET}"
    for bench in $BENCHMARKS; do
        [ -f "${bench}.go" ] || continue
        echo -n "  $bench.go ... "
        if $GO build -o "${bench}_go" "${bench}.go" 2>/dev/null; then
            echo "ok"
        else
            echo "FAILED"
        fi
...
# Header
printf "\n${BOLD}%-20s %10s %10s %10s %10s %10s %10s %10s %10s${RESET}\n" \
    "Benchmark" "Strada" "Go" "VM" "Perl" "Python" "Ruby" "Node" "PHP"
...
for bench in $BENCHMARKS; do
    ...
    # Go (timed binary, compiled above)
    if lang_enabled go && [ -x "./${bench}_go" ]; then
        gt=$(best_time $RUNS "./${bench}_go")
        printf " %9ss" "$gt"
    else
        gt=""
        printf " %10s" "SKIP"
...
echo ""
printf "${BOLD}%-20s %14s %14s %14s %14s %14s %14s %14s${RESET}\n" \
    "Speedup (vs Strada)" "Go" "VM" "Perl" "Python" "Ruby" "Node" "PHP"
...
for bench in $BENCHMARKS; do
    ...
    for lang in go vm perl python ruby node php; do
...
# Cleanup compiled Strada and Go binaries
for bench in $BENCHMARKS; do
    rm -f "./$bench" "./${bench}_go"
done
